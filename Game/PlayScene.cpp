...
void PlayScene::ChooseMap(int whatMap)
{
	...
	if (grid == NULL)
		...
		grid->ResetGrid(mapWidth, mapHeight);
	totalObjectIntoGrid.clear();
	LoadSceneObjects();
	//if (idStage == 1000)
	//{
	//	player->Reset();
...
void PlayScene::GetObjectFromGrid()
{
	...
	listCoins.clear();
	listPipe.clear();
	grid->GetObj(game->GetCamPosX() , game->GetCamPosY(), listObjectLoad);
	for (int i = 0; i < listObjectLoad.size(); i++)
	{
		if (dynamic_cast<Leaf*>(listObjectLoad[i]))
			listLeaf.push_back(listObjectLoad[i]);
		if (dynamic_cast<Coin*>(listObjectLoad[i]))
			listCoins.push_back(listObjectLoad[i]);
		if (dynamic_cast<Pipe*>(listObjectLoad[i]))
			listPipe.push_back(listObjectLoad[i]);
		if (dynamic_cast<Item*>(listObjectLoad[i]))
			listitems.push_back(listObjectLoad[i]);
		if (dynamic_cast<Enemy*>(listObjectLoad[i]))
...
void PlayScene::_ParseSection_OBJECTS(string line)
{
	...
	case OBJECT_TYPE_BRICK:
	{
		...
		obj->SetPosition(x, y);
		//listObjects.push_back(obj);
		totalObjectIntoGrid.push_back(obj);
...
void PlayScene::LoadSceneObjects()
{
	...
	grid->PutObjIntoGrid(totalObjectIntoGrid);
